...
- **`src/ui/`** is a small hash-routed vanilla-TS front end; `src/i18n/` holds the (English/Catalan) UI strings and locale persistence. Besides person records (`#seed:region:village:person`), the nobility has fixed URLs of its own: `#seed:region:royal` (the royal line), `#seed:region:royal:reign` (one sovereign's page), `#seed:region:village:house` (a manor's noble house), and `#seed:region:village:lord|baron:head` (one lord's page). Everything the UI names should be a link: kings and lords mentioned in chronicle prose link to their *own pages* through the same `EventRef` mechanism that links people (`route` + `routeIdx`), with aliases like "King Henry" resolved year-aware to the sovereign actually reigning at the event's date.
