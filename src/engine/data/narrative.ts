import type { DeathCause, SocialClass, TextureEvent, WorldEvent } from "../types.js";
...
export const DEATH_DETAIL: Record<DeathCause, string[]> = {
...
export const CAUSE_LABEL: Record<DeathCause, string> = { plague: "Plague", famine: "Famine", war: "War", infancy: "Infancy", childhood: "Childhood disease", childbirth: "Childbirth", disease: "Disease", oldage: "Old age" };

export const FATHER_OCC: Record<SocialClass, string[]> = {
...
export const WORLD_EVENTS: WorldEvent[] = [
  [1319, 1321, ["england", "france", "germany"], 6, 0.8, "hardship", SRC.account, p => `The great murrain struck: cattle and oxen died across the whole country, the plough-teams with them. The manorial account for those years records the herd ${CLASS_INFO[p.cls].wealth <= 2 ? "lost and the land half-tilled for two seasons" : "replaced at ruinous cost"}.`],
  [1356, 1356, ["germany"], 4, 0.6, "hardship", SRC.chron, () => `On St Luke's day the earth shook — the great earthquake that threw down the towers of Basel. Even at a distance walls cracked and everyone slept out of doors for a week.`],
  [1362, 1362, ["england", "germany"], 4, 0.5, "hardship", SRC.chron, () => `The Great Wind of January 1362 — the storm the chronicles called Grote Mandrenke — tore the roof from the church and flattened barns across the district.`],
  [1378, 1380, null, 12, 0.5, "life", SRC.chron, () => `Word came that there were now two popes, one at Rome and one at Avignon, each cursing the other. The priest could not say which was the true one; folk paid their tithes and hoped God would sort it out.`],
  [1391, 1391, ["catalonia"], 8, 0.55, "war", SRC.chron, () => `That summer mobs attacked the Jewish quarters in the cities — Barcelona, Girona, València. Many were killed or forced to the font; the calls stood half-empty ever after. It was spoken of for a generation.`],
  [1407, 1408, null, 4, 0.4, "hardship", SRC.chron, () => `The great frost of that winter: rivers froze to the bed, birds fell dead from the trees, and wine was cut with an axe. The old said no winter like it had been known.`],
  [1414, 1418, ["germany"], 12, 0.45, "life", SRC.chron, () => `The great Council sat at Constance to heal the Schism — three popes deposed or resigned, one elected, and the Bohemian preacher Hus burned, which set Bohemia alight.`],
  [1428, 1428, ["catalonia"], 4, 0.7, "hardship", SRC.chron, () => `On Candlemas the earth shook — the terratrèmol that ruined churches from the Pyrenees to Girona, where the falling vault killed worshippers at mass. Aftershocks came for weeks and people confessed in the streets.`],
  [1429, 1431, ["france"], 10, 0.6, "life", SRC.chron, () => `News ran through the country of the Maid — Jeanne, the peasant girl who raised the siege of Orléans and led the Dauphin to be crowned at Reims. Two years later came the word that the English had burned her at Rouen.`],
  [1450, 1450, ["italy"], 15, 0.4, "life", SRC.chron, () => `A Jubilee year at Rome: the roads south filled with pilgrims from every nation. So great was the crush that hundreds died on the bridge of Sant'Angelo.`],
  [1453, 1454, null, 12, 0.5, "life", SRC.chron, () => `Terrible news came from the east: Constantinople, the great city of the Greeks, had fallen to the Turk. Preachers called for crusade; princes promised much and did nothing.`],
  [1456, 1456, null, 6, 0.5, "life", SRC.chron, () => `A great comet burned in the sky for weeks — a hairy star, taken by all as an omen. The Pope ordered prayers against it and against the Turk in the same breath.`],
...
export const CHILD_EVENTS: TextureEvent[] = [
...
export const YOUTH_EVENTS: TextureEvent[] = [
...
export const ADULT_EVENTS: TextureEvent[] = [
...
export const COURT_CAUSES: string[] = ["letting beasts stray into the lord's corn", "brewing ale against the assize", "an affray with a neighbour over a boundary stone", "gleaning before the sheaves were carried", "taking firewood from the lord's coppice"];
export const FEMALE_EVENTS: TextureEvent[] = [
...
export const OLD_EVENTS: TextureEvent[] = [
