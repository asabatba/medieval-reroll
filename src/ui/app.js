...
import { buildRecordHTML, locator } from "./render.js";
...
export function initApp() {
  ...
  let worldSeed = 1444;
  let stack = [];   // [{regionKey, villageIdx, personId, label}]
  ...
  function render() {
    ...
    out.querySelectorAll("[data-goto]").forEach(b => b.addEventListener("click", () => {
      const [rk, vi, pid] = b.dataset.goto.split(":");
      const addr = { regionKey: rk, villageIdx: +vi, personId: +pid };
    ...
    out.querySelectorAll(".crumb[data-jump]").forEach(b => b.addEventListener("click", () => {
